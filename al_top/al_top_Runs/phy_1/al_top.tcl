...
set rc [catch {
  open_project {al_top.prj}
  import_device eagle_s20.db -package EG4S20BG256
  import_db {../syn_1/al_top_gate.db}
  read_sdc
  place
  export_db {al_top_place.db}
} RESULT]
if {$rc} {
...
set rc [catch {
  route
  report_area -io_info -file al_top_phy.area
  export_db {al_top_pr.db}
} RESULT]
if {$rc} {
...
set rc [catch {
  export_bid al_top_inst.bid
  bitgen -bit "al_top.bit"
} RESULT]
if {$rc} {
  step_error bitgen
  return -code error $RESULT
